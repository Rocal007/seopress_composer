...
add_shortcode('KOMPETENZWORTE', 'kompetentworte');
...
add_shortcode('HANDWERKSLEISTUNGENLISTE', 'handwerksleistungenliste');
...
function get_questions_build_faq($faq_cat, $how_many_faqs, $extra=False) {
    ...
    if (have_rows('fragen-'.$faq_cat, $page_id)) {
        while (have_rows('fragen-'.$faq_cat, $page_id)) : the_row();
        	...
        	$bl = show_bl();
        	$bez = show_bez(); //funkt mit dem echten bezirk erst, wenn es in einer kategorie ist
            ...
            if ($i < $how_many_faqs && $extra) {
				...
				echo '<div class="faq-container col-md-11">';
